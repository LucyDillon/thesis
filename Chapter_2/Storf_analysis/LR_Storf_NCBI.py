...
# read in storfs:
Storf = pd.read_csv("myPYinputfile.csv", names=['genome_id', 'Gene', 'GeneHit'])

Storf_drugclass = pd.read_csv("uniq_genes_drugclass.txt", sep='\t', names=['Gene', 'AMRFinderDrugClass'])

NCBI_storf_drugclass = pd.merge(Storf, Storf_drugclass, on='Gene')

NCBI_storf_all = [AMRFinder_all,NCBI_storf_drugclass]
NCBI_ALL_AND_STORF = pd.concat(NCBI_storf_all)
...
test_data = pd.merge(average_Overall_EUCAST_data, Overall_EUCAST_data, how='left',
                     left_on=['genome_id','antibiotic'],
...
AMRFinder_EUCAST = pd.merge(left=NCBI_ALL_AND_STORF, right=DrugClass_info, on='AMRFinderDrugClass')
...
def antibiotic_classification(antibiotic):
    antibiotic =AMRFinder_final[AMRFinder_final['antibiotic'] == antibiotic]
...
ab_list = ['ciprofloxacin', 'erythromycin', 'tetracycline', 'gentamicin', 'meropenem',
 'amikacin', 'levofloxacin', 'tobramycin', 'colistin', 'doripenem',
 'ceftriaxone', 'aztreonam', 'cefepime', 'ertapenem',
 'imipenem', 'chloramphenicol',  'moxifloxacin', 'fosfomycin',
 'clindamycin']
