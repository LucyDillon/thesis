...
# Read in storfs:
Resfinder_storf = pd.read_csv("myPYinputfile.csv", names=['genome_id', 'Gene', 'GeneHit'])

Resfinder_storf_drugclass = pd.merge(Resfinder_storf, Resfinder_drugclass, on='Gene')
Resfinder_storf_all = [resfinder_all,Resfinder_storf_drugclass]
Resfinder_ALL_AND_STORF = pd.concat(Resfinder_storf_all)
